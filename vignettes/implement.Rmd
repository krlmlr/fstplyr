...
1. A constructor, `src_fst()`, that accepts a `path` to an existing directory. (This is the only exported function.) This function calls `dplyr::src(subclass = "fst")` (which really should be named `dplyr::new_src()`) and already collects the metadata of all `.fst` files. The function returns an object of class `"src_fst"`.
...
So far, only `select()` and `rename()` are more efficient than working on data frames. Implementing better versions of the other verbs is now only a matter of diligence. In particular, `slice()` and `filter()` should be fairly easy to implement, the latter perhaps with the help of the [*bindr*](https://krlmlr.github.io/bindr/) package.

I have no idea why `R CMD check` currently gives me S3 method consistency headaches, but I'm pretty sure this is fixable:
